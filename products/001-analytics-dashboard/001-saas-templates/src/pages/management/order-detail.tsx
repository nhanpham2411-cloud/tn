  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center gap-md py-4xl">
        <Thumbnail type="icon" shape="circle" size="lg" color="surface" icon={<Package className="size-[22px]" />} className="mb-sm" />
                        ...
                        <TableRow key={i} className="group">
                          <TableCell>
                            <div>
                              ...
                              <p className="sp-caption text-muted-foreground">{item.productId}</p>
                            </div>
                          </TableCell>
                      ...
                      {timeline.map((step, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-start gap-md relative">
                            <Thumbnail type="icon" size="sm" color={step.done ? "primary" : "default"} icon={<StepIcon className="size-[14px]" />} className="!size-[30px] z-10" />
                            <div className="flex-1 flex flex-col gap-2xs sm:flex-row sm:items-center sm:justify-between py-xs min-h-[30px]">
                              ...
                              <span className="sp-caption text-muted-foreground shrink-0">{step.date}</span>
                            </div>
                          </div>
                        )
                      })}
                ...
                <DCard>
                  ...
                  {notes.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-sm">
                      {notes.map((note) => (
                        <div key={note.id} className="rounded-xl bg-surface-raised dark:bg-surface-inset p-lg">
                          <div className="flex items-center justify-between mb-xs">
                            <span className="sp-body-semibold text-foreground">{note.author}</span>
                            <span className="sp-caption text-muted-foreground">{note.date}</span>
                          </div>
            ...
            {refreshing ? (
              ...
              <>
                <DCard>
                  ...
                  <p className="sp-caption text-muted-foreground mt-2xs">ID: {order.customerId}</p>
                </DCard>
        ...
        <AlertDialog open={cancelDialog} onOpenChange={setCancelDialog}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive" onClick={handleCancel}>
                Cancel Order
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
